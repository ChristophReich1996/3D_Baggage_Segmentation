...
class Network_Generator():
                    ...
                    for vol_y in range(0, volume_in[0].shape[2], win_size):
                        for vol_z in range(0, volume_in[0].shape[3], win_size):
                            ...
                            if down_fact > 0 and side_len_down > 0:
                                ...
                                pad_x_back = max(down_fact * side_len_down - (volume_in[0].shape[1] - x_start_down),0)
                                pad_y_back = max(down_fact * side_len_down - (volume_in[0].shape[2] - y_start_down),0)
                                pad_z_back = max(down_fact * side_len_down - (volume_in[0].shape[3] - z_start_down),0)

                                volume_down = np.pad(volume_in[0], ((0,0),(pad_x_front, pad_x_back), (pad_y_front, pad_y_back), (pad_z_front, pad_z_back)))
                                ...
                                volume_down = nn.functional.avg_pool3d(volume_down, down_fact, down_fact)
                                volume_down = torch.unsqueeze(volume_down, dim=0).float()
                            ...
                            while active > 0 and side_len >= 1:
                                ...
                                acts = [self._oj_model.inference(volume.to(device), coord.to(device), volume_down.to(device)) for coord in coords]
                                # Check for hits
        ...
        with torch.no_grad():
            ...
            for j, batch in enumerate(loader_val):
                ...
                if j % 8 == 7:
                    ...
                    self._oj_model.eval()
                    yhat = self._oj_model(volume.to(device), coords.to(device), volume_down.to(device))
                    # Computes validation loss
                    loss_val_batch = self._oj_loss(yhat, labels.to(device)).item()
                    if iou:
                        loss_iou_batch = layers.IOU(coords, yhat, labels, volume.shape[0])
    ...
    def train(self, train_dataset, val_dataset, side_len, npoints, name, load=False, 
        ...
        def _step_train(batch):
            ...
            yhat = self._oj_model(volume.to(device), coords.to(device), volume_low.to(device))
        ...
        for _ in range(self._size_iter):
            ...
            for i_, batch in enumerate(loader_train):
                ...
                for j in range(1):
                    ...
                    t = utils.Timer()
                    batch_in = many_to_one_collate_fn_sample(samples, down=True)
                    #print("Collate", t.stop(), flush=True)
                    t = utils.Timer()
                    loss_train_batch, mse_train_batch = _step_train(batch_in)
                    #print("Step", t.stop(), flush=True)
...
class Res_Auto_3d_Model_Occu(nn.Module):
    def __init__(self):
        ...
        self.encode_low = nn.Sequential(layers.Res_Block_Down_3D(1, 64, 3, 1, nn.SELU(), False),
                                        layers.Res_Block_Down_3D(64, 64, 3, 1, nn.SELU(), True),
                                        layers.Res_Block_Down_3D(64, 64, 3, 1, nn.SELU(), False),
                                        layers.Res_Block_Down_3D(64, 64, 3, 1, nn.SELU(), True),
                                        layers.Res_Block_Down_3D(64, 3, 3, 1, nn.SELU(), True))

        self.decode = nn.Sequential(layers.Res_Block_Up_Flat(192 + 192 + 3, 512, nn.SELU()),
    ...
    def forward(self, volume, coords, volume_low):
        ...
        encoded_high = torch.repeat_interleave(encoded_high, int(coords.shape[0]/volume.shape[0]), dim=0)
        encoded_low = self.encode_low(volume_low)
        encoded_low = encoded_low.view(encoded_low.shape[0],-1)
        encoded_low = torch.repeat_interleave(encoded_low, int(coords.shape[0]/volume.shape[0]), dim=0)

        out = self.decode(torch.cat((encoded_high, encoded_low, coords), dim=1))
        return out
