...
class WeaponDatasetGeneratorLowRes():
    ...
    def generate_data(self):
        for index in range(len(self.data)):
            ...
            volume_n= torch.from_numpy(volume_n).to(device)[:,0:self.dim_max,:,:]
            volume_n = nn.functional.pad(volume_n,(0,0,0,0,0,self.dim_max-volume_n.shape[1]))
            volume_pooled_tg = nn.functional.max_pool3d(volume_n, self.side_len, self.side_len)
            ...
            labels_n = np.pad(labels_n, ((offsets_n[0], 0),(offsets_n[1], 0),(offsets_n[2], 0)))
            labels_n = torch.unsqueeze(torch.from_numpy(labels_n).to(device).float(), dim=0)[:,0:self.dim_max,:,:]
            labels_n = nn.functional.pad(labels_n,(0,volume_n.shape[3]-labels_n.shape[3],0,volume_n.shape[2]-labels_n.shape[2], 0,self.dim_max-labels_n.shape[1]))
            labels_n = nn.functional.max_pool3d(labels_n, self.side_len, self.side_len)
...
def sample(volume_n, label_n, npoints=2**10, side_len=32, sampling='one', down_fact=0, side_len_down=0, test=False):
        ...
        if down_fact > 0 and side_len_down > 0:
            ...
            pad_x_back = max(down_fact * side_len_down - (sampling_shapes_tc[1] - x_start_down),0)
            pad_y_back = max(down_fact * side_len_down - (sampling_shapes_tc[2] - y_start_down),0)
            pad_z_back = max(down_fact * side_len_down - (sampling_shapes_tc[3] - z_start_down),0)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create and Check High and Low Resolution Weapon Datasets')
