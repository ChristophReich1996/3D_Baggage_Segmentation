...
class WeaponDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        volume_with_labels_tc = torch.from_numpy(np.expand_dims(np.concatenate((volume_n, labels_expanded_n), axis = 0), axis=0))#.to(device)
        t3a = utils.Timer()
        volume_with_labels_pooled_tc = nn.functional.max_pool3d(volume_with_labels_tc, self.side_len, self.side_len)
        print("Downsampling", t3a.stop())
        ...
        print("Downsampling and Labeling", t3.stop())
